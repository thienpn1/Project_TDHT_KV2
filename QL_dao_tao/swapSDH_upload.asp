...
<FORM ACTION = "#" ENCTYPE="multipart/form-data" METHOD="POST">
	...
	<br>
	* Chú ý phải điền đầy đủ các cột như trong file mẫu [<a href="DangKy.xls"><font color=red>Tải file mẫu</font></a>]<br>
				...
				if not adoRs is nothing then
					while not adoRs.eof	
						if (Server.HTMLEncode(Trim(Cstr(adoRs.Fields(1).Value&" ")))<>"") and _
						   ...
						   (Server.HTMLEncode(Trim(Cstr(adoRs.Fields(8).Value&" ")))<>"") and _	
						   (Server.HTMLEncode(Trim(Cstr(adoRs.Fields(10).Value&" ")))<>"") and _	
						   (Server.HTMLEncode(Trim(Cstr(adoRs.Fields(11).Value&" ")))<>"") then		
						   
							sql= " INSERT INTO timesheet.tbl_swapsdh "&_
									" (province, link_label, link_type,cabin, account_ftth, viba, point_old, point_new, port_detect, "&_
									" CR, date_action, nodename, BSC, Sub_old, Slot_old, Sub_new, Slot_new, Port_new, user_province, user_zone, note)"&_
								 " VALUES ('"&_
										 ...
										 pdb(Server.HTMLEncode(Trim(Cstr(adoRs.Fields(21).Value&" "))))&"')"
							'Response.write(sql)							
							objConn.Execute(sql)
							count = count + 1
						else				
